...
- **FlyoutWindow** is a popup that displays launcher items. Supports three view modes per launcher: list view (icon + name side by side), icon grid view (larger icon with name below, configurable icons-per-row wrapping grid), and a compact small-icon grid that matches tray-sized tiles without per-item text labels. One instance is maintained per launcher (`_instances: Dictionary<string, FlyoutWindow>`). Shown from a launcher's tray icon click, positioned above the taskbar, and when `UserSettings.FlyoutAnimationsEnabled` is on the full rendered window slides in and out from its anchored edge using short eased position updates, dismissed on focus loss or Escape. It also supports direct drag-and-drop reordering using custom `ListView` drag handlers and insertion indicators, and in icon mode dragging near the flyout's left or right edge snaps the layout wider or narrower by whole icon columns and persists `Launcher.IconModeIconsPerRow` immediately.
...
- **Add/extend icon gallery tabs:** Edit `Classes/IconGallery.cs` — add entries to `FluentIconCategories` or `EmojiCategories` arrays, or extend the manual image-source tabs. The item gallery supports Segoe Fluent Icons (PUA glyphs), emojis, bundled app color icons, and a remote selfh.st catalog tab; launcher tray icon selection supports presets, glyphs, emojis, and the selfh.st tab for image-based custom icons. Icon selection uses a pending-selection model with Confirm/Cancel buttons. Selected icons get an accent border. Color swatches override `ButtonBackgroundPointerOver`/`ButtonBackgroundPressed` to keep their color on hover. When opened, the gallery pre-selects the current icon (correct tab, color swatch, and icon button) so the user can change just the color without re-finding the icon.
