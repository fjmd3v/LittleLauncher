...
- **Icon view** (ViewMode = 0, default): Each column is a dynamically created `ScrollViewer` containing icon tiles in a wrapping grid. `Launcher.IconModeIconsPerRow` controls the maximum icons shown across each icon-mode column (default 3, configurable from 1 to 6), and the flyout shrinks each column to the widest row actually present so sparse layouts do not keep extra horizontal whitespace. While an icon-mode flyout is open, the user can drag its left or right edge to snap the layout wider or narrower by whole icon columns without opening settings.
